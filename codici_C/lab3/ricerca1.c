...
{
printf("Quanto è lunga a1?\n");
scanf("%d",&lung1);

if(lung1>max)
   do{
	   printf("a1 non può avere più di %d elementi\nPrego, inserire la lunghezza di a1\n",max);
	   scanf("%d",&lung1);
...
printf("\nCONTROLLO:La stinga a1 è composta dagli elementi:\n");
for(i=0;i<lung1;i++)
    printf("%d; ",a1[i]);
...
if(lung1>(lung1-1))
   do{
	   printf("a2 non può avere più di %d elementi\nPrego, inserire la lunghezza di a2\n",lung1-1);
	   scanf("%d",&lung2);
...
printf("\nCONTROLLO:La stinga a2 è composta dagli elementi:\n");
for(i=0;i<lung2;i++)
    printf("%d; \n",a2[i]);
}
