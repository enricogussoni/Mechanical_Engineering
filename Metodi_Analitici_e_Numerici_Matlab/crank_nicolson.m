...
%[t_h,u_h,iter_pf]=crank_nicolson(f,df,t_max,y_0,h)
...
% F(u) = u_n + (h/2)*[f(t_n,u_n) + f(t_{n+1},u)] - u = 0
% con il metodo di Newton. Per questo � necessario passare
% in input l'espressione di df/dy, in modo da creare
% dF(u)/du = (h/2)* df/dy(t_{n+1},u) - 1.
%
% Input:
...
for it=2:N
    ...
    phi  = @(u) u_old + (h/2)*( f(t_old,u_old) + f(t_new, u) ) - u;
    dphi = @(u) (h/2)*df(t_new, u) - 1;
