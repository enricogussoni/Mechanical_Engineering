...
P_AC = zeros(length(lambda_vect),length(vAC));

%for kk = 1:length(lambda_vect)
    for j=1:length(vAC)
        omega(j) = lambda_d*vAC(j)/R; %lambda_vect(kk)*vAC(j)/R;
        ...
        PowerAC1(j)=ro_aria*Nb*omega(j)*sum(cc)/2;
        cc2=ones(length(r),1);
        for i=1:length(r)
        ...
        PowerAC(j) = (PowerAC1(j)+PowerAC2(j))/2;
...
for j=1:length(vCD) 
    ...
    dbetac_vect(j)=abs(betac(j)-betacd(j));
end
...
for j=1:length(vDE)
    ...
    dbetac2_vect(j) = abs(betac2(j)-betacd(j));
end 

figure(888)
plot([zeros(1,length(CpAC)) dbetac_vect dbetac2_vect],[CpAC CpCD CpDE]); hold on;
title('Variation of Cp with beta_c'); xlabel('beta_c'); ylabel('Cp');
