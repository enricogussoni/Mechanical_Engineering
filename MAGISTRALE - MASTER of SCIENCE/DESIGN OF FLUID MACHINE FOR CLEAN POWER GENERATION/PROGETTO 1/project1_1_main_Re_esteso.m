...
r_T = 0.96;
r = r_H:0.02:r_T;
r_cut = (0.7:0.05:0.9); % settore di riferimento per linearizzare

tol=1e-5;

% from wind.nrel.gov/airfoils/AirfoilFamilies for rotor R up to 50m HUB NREL S818, PRIM NREL S827, TIP NREL S828
% Hub
load A__NREL_S818_Re5e5.txt; load B__NREL_S818_Re1e6.txt; load New_C__NREL_S818_Re25e6.txt; load New_D__NREL_S818_Re35e6.txt;
% Primary 
load E__NREL_S827_Re5e5.txt; load F__NREL_S827_Re1e6.txt; load New_G__NREL_S827_Re25e6.txt; load New_H__NREL_S827_Re35e6.txt;
% Tip
load I__NREL_S828_Re5e5.txt; load L__NREL_S828_Re1e6.txt; load New_M__NREL_S828_Re25e6.txt; load New_N__NREL_S828_Re35e6.txt;

alfa=[-4:0.5:12.5,13.5:0.5:16];
hub1=[posvet(alfa,A__NREL_S818_Re5e5(:,1),A__NREL_S818_Re5e5(:,2));posvet(alfa,A__NREL_S818_Re5e5(:,1),A__NREL_S818_Re5e5(:,3))];
hub2=[posvet(alfa,B__NREL_S818_Re1e6(:,1),B__NREL_S818_Re1e6(:,2));posvet(alfa,B__NREL_S818_Re1e6(:,1),B__NREL_S818_Re1e6(:,3))];
hub3=[posvet(alfa,New_C__NREL_S818_Re25e6(:,1),New_C__NREL_S818_Re25e6(:,2));posvet(alfa,New_C__NREL_S818_Re25e6(:,1),New_C__NREL_S818_Re25e6(:,3))];
hub4=[posvet(alfa,New_D__NREL_S818_Re35e6(:,1),New_D__NREL_S818_Re35e6(:,2));posvet(alfa,New_D__NREL_S818_Re35e6(:,1),New_D__NREL_S818_Re35e6(:,3))];
pri1=[posvet(alfa,E__NREL_S827_Re5e5(:,1),E__NREL_S827_Re5e5(:,2));posvet(alfa,E__NREL_S827_Re5e5(:,1),E__NREL_S827_Re5e5(:,3))];
pri2=[posvet(alfa,F__NREL_S827_Re1e6(:,1),F__NREL_S827_Re1e6(:,2));posvet(alfa,F__NREL_S827_Re1e6(:,1),F__NREL_S827_Re1e6(:,3))];
pri3=[posvet(alfa,New_G__NREL_S827_Re25e6(:,1),New_G__NREL_S827_Re25e6(:,2));posvet(alfa,New_G__NREL_S827_Re25e6(:,1),New_G__NREL_S827_Re25e6(:,3))];
pri4=[posvet(alfa,New_H__NREL_S827_Re35e6(:,1),New_H__NREL_S827_Re35e6(:,2));posvet(alfa,New_H__NREL_S827_Re35e6(:,1),New_H__NREL_S827_Re35e6(:,3))];
tip1=[posvet(alfa,I__NREL_S828_Re5e5(:,1),I__NREL_S828_Re5e5(:,2));posvet(alfa,I__NREL_S828_Re5e5(:,1),I__NREL_S828_Re5e5(:,3))];
tip2=[posvet(alfa,L__NREL_S828_Re1e6(:,1),L__NREL_S828_Re1e6(:,2));posvet(alfa,L__NREL_S828_Re1e6(:,1),L__NREL_S828_Re1e6(:,3))];
tip3=[posvet(alfa,New_M__NREL_S828_Re25e6(:,1),New_M__NREL_S828_Re25e6(:,2));posvet(alfa,New_M__NREL_S828_Re25e6(:,1),New_M__NREL_S828_Re25e6(:,3))];
tip4=[posvet(alfa,New_N__NREL_S828_Re35e6(:,1),New_N__NREL_S828_Re35e6(:,2));posvet(alfa,New_N__NREL_S828_Re35e6(:,1),New_N__NREL_S828_Re35e6(:,3))];
...
while abs(Re_H-Re)>tol
    ...
    while abs(a_ph-a_pold)>tol
        a_pold=a_ph;
        phi=atan((1-a(x==x_H))/((1+a_ph)*x_H)); %!!!non posso usare il semplice rapporto a'/a perchè il triangolo delle velocità indotte non è parallelo alla direzione del lift con il drag, quindi uso sempre il triangolo effettivo.
        W=(1-a(x==x_H))*V0/sin(phi);
        f=Nb*(R-r_H*R)/(2*r_H*R*sin(phi));
        F=2/pi*acos(exp(-f));
        if Re<=5e5  
                RE=5e5; medH=[hub1(1,:);hub1(2,:)]; 
            elseif Re>5e5 && Re<=1e6
                    RE=Re; Re1=5e5; Re2=1e6;
                    medH=Re_medio(hub1(1,:),hub2(1,:),hub1(2,:),hub2(2,:),RE,Re1,Re2);
            elseif Re>1e6 && Re<=2.5e6
                    RE=Re; Re1=1e6; Re2=2.5e6;
                    medH=Re_medio(hub2(1,:),hub3(1,:),hub2(2,:),hub3(2,:),RE,Re1,Re2);
            elseif Re>2.5e6 && Re<=3.5e6
                    RE=Re; Re1=2.5e6; Re2=3.5e6;
...
% Dalla chord opt linearizzo nell' intorno di r/R = 0.8. TBN: "it's
% kitchen!" 
...
while abs(a_pH-a_pold)>tol 
    ...
    while abs(aH-aold)>tol
        ...
        if ReyH<=5e5  
                RE=5e5; medH=[hub1(1,:);hub1(2,:)]; 
        elseif ReyH>5e5 && ReyH<=1e6
                RE=ReyH; Re1=5e5; Re2=1e6;
                medH=Re_medio(hub1(1,:),hub2(1,:),hub1(2,:),hub2(2,:),RE,Re1,Re2);
        elseif ReyH>1e6 && ReyH<=2.5e6
                RE=ReyH; Re1=1e6; Re2=2.5e6;
                medH=Re_medio(hub2(1,:),hub3(1,:),hub2(2,:),hub3(2,:),RE,Re1,Re2);
        elseif ReyH>2.5e6 && ReyH<=3.5e6
                RE=ReyH; Re1=2.5e6; Re2=3.5e6;
                medH=Re_medio(hub3(1,:),hub4(1,:),hub3(2,:),hub4(2,:),RE,Re1,Re2);
        elseif ReyH>3.5e6
            RE=3.5e6; medH=[hub4(1,:);hub4(2,:)];
        end
...
while abs(a_pP-a_pold)>tol 
    ...
    while abs(aP-aold)>tol
        ...
        if ReyP<=5e5  
                RE=5e5; medP=[pri1(1,:);pri1(2,:)]; 
        elseif ReyP>5e5 && ReyP<=1e6
                RE=ReyP; Re1=5e5; Re2=1e6;
                medP=Re_medio(pri1(1,:),pri2(1,:),pri1(2,:),pri2(2,:),RE,Re1,Re2);
        elseif ReyP>1e6 && ReyP<=2.5e6
                RE=ReyP; Re1=1e6; Re2=2.5e6;
                medP=Re_medio(pri2(1,:),pri3(1,:),pri2(2,:),pri3(2,:),RE,Re1,Re2);
        elseif ReyP>2.5e6 && ReyP<=3.5e6
                RE=ReyP; Re1=2.5e6; Re2=3.5e6;
                medP=Re_medio(pri3(1,:),pri4(1,:),pri3(2,:),pri4(2,:),RE,Re1,Re2);
        elseif ReyP>3.5e6
            RE=3.5e6; medP=[pri4(1,:);pri4(2,:)];
        end
...
while abs(a_pT-a_pold)>tol 
    ...
    while abs(aT-aold)>tol
        ...
        if ReyT<=5e5  
                RE=5e5; medT=[tip1(1,:);tip1(2,:)]; 
        elseif ReyT>5e5 && ReyT<=1e6
                RE=ReyT; Re1=5e5; Re2=1e6;
                medT=Re_medio(tip1(1,:),tip2(1,:),tip1(2,:),tip2(2,:),RE,Re1,Re2);
        elseif ReyT>1e6 && ReyT<=2.5e6
                RE=ReyT; Re1=1e6; Re2=2.5e6;
                medT=Re_medio(tip2(1,:),tip3(1,:),tip2(2,:),tip3(2,:),RE,Re1,Re2);
        elseif ReyT>2.5e6 && ReyT<=3.5e6
                RE=ReyT; Re1=2.5e6; Re2=3.5e6;
                medT=Re_medio(tip3(1,:),tip4(1,:),tip3(2,:),tip4(2,:),RE,Re1,Re2);
        elseif ReyT>3.5e6
            RE=3.5e6; medT=[tip4(1,:);tip4(2,:)];
        end
...
TotPower2=4*ro_aria*pi*(R^2)*(V0^3)*sum(Cx2)/(lambda_d^2);
...
Fa=sum(fx)/1000;Fa2=sum(fx2)/1000;Ct=Fa*1000/(ro_aria*pi*(R^2)*(V0^2)/2);
