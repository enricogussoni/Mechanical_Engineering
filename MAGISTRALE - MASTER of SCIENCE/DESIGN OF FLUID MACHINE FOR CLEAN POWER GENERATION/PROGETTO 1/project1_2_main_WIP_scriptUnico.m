...
Vc = V0; %.txt
Vr = 15; % % m/s from "Wind Energy Handbook JW and sons, 2001"
step_v = 1;
vAC = (Vcutin+2):step_v:Vc; % partiamo 2 dopo cutin per questione vista in classe a slide 4
...
e=0.5; % Indice per convergenza di a e a'
tol = 0.001; % !!! minore di quella usata fin'ora: la precizione su a e a' è alla 4a decimale !!!

%% RANGE A-C -> [Vcutin, Vc]
for j=1:length(vAC)
    omega(j) = lambda_d*vAC(j)/R;
    xACj = (omega(j)*(r*R)/vAC(j))'; % non faccio xAC(j) perché non serve salvarsi la matrice, sovrascrive j-esimo vettore
    x_H = xACj(r==r_H);
    x_P = xACj(r==r_P);
    x_T = xACj(r==r_T);
    a = ad;
    ...
    while abs(a_pH-a_pold)>tol 
        nh=nh+1;
        a_pold=a_pH;
        while abs(aH_new-aold)>tol
            aold=aH;
            PHI=atan((1-aH)/((1+a_pH)*x_H));
            Winf=(1-aH)*vAC(j)/sin(PHI);
            f=Nb*(R-r_H*R)/(2*r_H*R*sin(PHI));
            F=2/pi*acos(exp(-f));
            ReyH=Winf*cordalin(xACj==x_H)/ni;
                if ReyH<=1e6
                    RE=ReyH;
                else
                    RE=1e6;
            ...
            medH=Re_medio(hub1(1,:),hub2(1,:),hub1(2,:),hub2(2,:),RE,1e6,5e5);
%             beta_inf=PHI*180/pi - beta_cH; %TBN: beta_cH di design %
%             Kinematic similarity -> beta_inf doesn't change
%             
%             beta_inf=round(beta_inf,1); % Arrotondamento per trovare i dati
%                                         % di Cl e Cd che limita la precisione di tutto il ciclo
% 
%             Cl=posvet(beta_inf,alfa,medH(1,:));
%             Cd=posvet(beta_inf,alfa,medH(2,:));
            aH=sol(xACj==x_H)*Winf*(Cl/tan(PHI)+Cd)/(4*vAC(j)*F);
            aH_new=e*aH+(1-e)*aold;
            fprintf('check %f\n',abs(aH_new-aold));
        end
        fprintf('check ah\n');
        a_pH=sol(xACj==x_H)*Winf*(Cl-Cd/tan(PHI))/(4*omega(j)*r_H*R*F);
        a_pH=e*a_pH+(1-e)*a_pold;
    end
fprintf('check_hub\n');
    ...
    while abs(a_pP-a_pold)>tol 
        np=np+1;
        a_pold=a_pP;
        while abs(aP_new-aold)>tol
            aold=aP;
            PHI=atan((1-aP)/((1+a_pP)*x_H));
            Winf=(1-aP)*vAC(j)/sin(PHI);
            f=Nb*(R-r_H*R)/(2*r_H*R*sin(PHI));
            F=2/pi*acos(exp(-f));
            ReyP=Winf*cordalin(xACj==x_H)/ni;
                if ReyP<=1e6
                    RE=ReyP;
                else
                    RE=1e6;
            ...
            medP=Re_medio(pri1(1,:),pri2(1,:),pri1(2,:),pri2(2,:),RE,1e6,5e5);
%             beta_inf=PHI*180/pi - beta_cP; %TBN: beta_cP di design
%             
%             beta_inf=round(beta_inf,1); % Arrotondamento per trovare i dati
%                                         % di Cl e Cd che limita la precisione di tutto il ciclo
% 
%             Cl=posvet(beta_inf,alfa,medP(1,:));
%             Cd=posvet(beta_inf,alfa,medP(2,:));
            aP=sol(xACj==x_P)*Winf*(Cl/tan(PHI)+Cd)/(4*vAC(j)*F);
            aP_new=e*aP+(1-e)*aold;
            fprintf('check %f\n',abs(aP_new-aold));
        end
        fprintf('check aP\n');
        a_pP=sol(xACj==x_P)*Winf*(Cl-Cd/tan(PHI))/(4*omega(j)*r_P*R*F);
        a_pP=e*a_pP+(1-e)*a_pold;
    end
end
