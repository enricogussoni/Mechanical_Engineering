...
figure()
plot (r,sigma);
title('Solidity'); xlabel('r/R'); ylabel('\sigma');
hold on
    p1 = plot(r_H,sigmaH,'o','MarkerSize',5,'MarkerFaceColor','r'); 
    p2 = plot(r_P,sigmaP,'o','MarkerSize',5,'MarkerFaceColor','y'); 
    p3 = plot(r_T,sigmaT,'o','MarkerSize',5,'MarkerFaceColor','g');
hold off
...
figure()
plot(r,max(EFF));
title('Profile efficiency'); xlabel('r/R'); ylabel('e=L/D');
...
figure()
plot(r,Cl,r,Cd)
title('Cl and Cd along the balde'), xlabel('r/R'), legend('Cl','Cd');

figure()
plot(r,PHI)
title('Attack angle'); xlabel('r/R'), ylabel('Phi_inf');
...
for i=1:length(r)
    Cx2(i)=F(i)*a_pi(i)*(1-ai(i))*(x(i)^3)*dx;
     Cp2_vect(i)=8*sum(Cx2)/(x(i)^2);
end
Cp2=8*sum(Cx2)/(lambda_d^2);
TotPower2=4*ro_aria*pi*(R^2)*(V0^3)*sum(Cx2)/(lambda_d^2);

figure() 
%plot(x,Cp1_vect); hold on
plot(x,Cp2_vect); %hold off
title('Cp as function of local TSR'); xlabel('x'); ylabel('Cp');

% The two calculation must give the same results
Cpdev=abs(Cp1-Cp2)*100/Cp1; % Differenza percentuale tra i Cp con i due metodi
Powdev=abs(TotPower-TotPower2)*100/TotPower; % Differenza percentuale tra la potenze con i due metodi
Cp = (Cp1+Cp2)/2; % Adimensional vs Cp=0.48 as maximum value for statistics 
...
figure()
plot(r,fx,r,fx2)
title('Axial force'); xlabel('r/R'), ylabel('Fx');

figure()
plot(lambda_vect,Cp_vect)
title('Cp as function of TSR'); xlabel('TSR'); ylabel('Cp')
...
figure()
plot(r,a,r,a_p)
title('Induction coefficients'); xlabel('r/R'); legend('a','at');

figure()
plot(r,Re_vect)
title('Re number along r'); xlabel('r/R'); ylabel('Re')