...
V0 = 9; % m/s Lampedusa
Nb = 3; 
lambda_stat = 7; % from statistical diagr 
Cp_stat = 0.48; % from statistical diagr 

omega = lambda_stat*V0/R; % rad/s, first HP
rpm = omega*60/2/pi;
ro_aria=1.225; % kg/mc
...
while abs(Re_H-Re)>tol
    ...
    while abs(a_ph-a_pold)>tol
        ...
        a_ph=sigmaH*W*(Cl-Cd/tan(phi))/(4*omega*r_H*R*F);
    end
    Re_H=W*corda_H/ni;
...
while abs(Re_P-Re)>tol
    ...
    while abs(a_pp-a_pold)>tol
        ...
        a_pp=sigmaP*W*(Cl-Cd/tan(phi))/(4*omega*r_P*R*F);
    end
    Re_P=W*corda_P/ni;
...
while abs(Re_T-Re)>tol
    ...
    while abs(a_pt-a_pold)>tol
        ...
        a_pt=sigmaT*W*(Cl-Cd/tan(phi))/(4*omega*r_T*R*F);
    end
    Re_T=W*corda_T/ni;
...
for i=1:length(r)
    ...
    while abs(a_pi(i)-a_pold)>tol
        ...
        a_pi(i)=sigma(i)*W(i)*(Cl(i)-Cd(i)/tan(phi(i)))/(4*omega*r(i)*R*F(i));
    end
end
...
%% Opt chord - PLOTS
figure(1)
    plot(r,corda);hold on;
    p1 = plot(r_H,corda_H,'o');
    p2 = plot(r_P,corda_P,'o');
    p3 = plot(r_T,corda_T,'o');
    hold off;
    legend([p1 p2 p3],'hub','primary','tip');
    xlabel('r/R'); ylabel('chord [m]');
    title('optimized chord');
figure(2)
    plot(r,calettamento);hold on;
    p1 = plot(r_H,calettamento_H,'o');
    p2 = plot(r_P,calettamento_P,'o');
    p3 = plot(r_T,calettamento_T,'o');
    hold off;
    legend([p1 p2 p3],'hub','primary','tip');
...
corda_max=R*0.12;%structural constrain, checked graphically with figure(3)
pos_cut = find(sum(abs(r-r_cut')<1e-5)~=0);
corda_cut = corda(pos_cut); 
P = polyfit(r_cut,corda_cut,1);
cordalin = polyval(P,r);
%% Lin chord - PLOTS
figure (3)
    ...
    hold off;
    xlabel('r/R'); ylabel('chord [m]');
    title('Linearization of blade cord'); saveas(gcf,'LinChord2Plot','jpg')
%% Calculation of new inferferience coefficients
...
while abs(a_pH-a_pold)>tol 
    ...
    a_pH=sol(x==x_H)*Winf*(Cl-Cd/tan(PHI))/(4*omega*r_H*R*F);
...
while abs(a_pP-a_pold)>tol 
    ...
    a_pP=sol(x==x_P)*Winf*(Cl-Cd/tan(PHI))/(4*omega*r_P*R*F);
...
while abs(a_pT-a_pold)>tol 
    ...
    a_pT=sol(x==x_T)*Winf*(Cl-Cd/tan(PHI))/(4*omega*r_T*R*F);
    a_pT=e*a_pT+(1-e)*a_pold;
end

%% Profile charateristics along blade span
[CL, CD]=blade(medH(1,:),medH(2,:),medP(1,:),medP(2,:),medT(1,:),medT(2,:),r_H,r_P,r_T,r);
EFF=CL./CD;
BETAinf=ones(1,length(r));
for i=1:length(r)
...
while abs(a_pi(i)-a_pold)>tol 
    ...
    a_pi(i)=sol(i)*Winf(i)*(Cl(i)-Cd(i)/tan(PHI(i)))/(4*omega*r(i)*R*F(i));
    a_pi(i)=e*a_pi(i)+(1-e)*a_pold;
end
end

%% Calculation of maximum power and Cp using Force calculation
Cx=ones(length(r),1);
...
Cp=Nb*omega*sum(Cx)/(pi*R^2*V0^3);
TotPower=ro_aria*Nb*omega*sum(Cx)/2;
...
Cp2=8*sum(Cx2)/(lambda_stat^2);
TotPower2=4*ro_aria*pi*(R^2)*(V0^3)*sum(Cx2)/(lambda_stat^2);

figure() 
plot(x,Cx); hold on
plot(x,Cx2); hold off;
title('Cp as function of local TSR'); xlabel('x');
