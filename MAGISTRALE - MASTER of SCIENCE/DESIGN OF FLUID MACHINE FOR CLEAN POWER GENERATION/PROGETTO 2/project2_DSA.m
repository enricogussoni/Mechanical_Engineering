...
dang_vect = 180./Nstream_vect; % for discretization sensibility analysis

for qq=1:length(Nstream_vect)
Nstream=Nstream_vect(qq); %10; %20; %30; % choice
R=D/2; 
dang=dang_vect(qq);

%% Variation of v0 and lambda
vmin=3;vmax=15; %m/s
lambdamin=0.2;lambdamax=8; %speed ratio
V0=vmin:vmax;
Lambda=lambdamin:0.2:lambdamax;
%torque_ref=zeros(length(Lambda),length(Nstream_vect));
