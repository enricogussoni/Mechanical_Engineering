...
% load('DH1.mat')
% figure(1)
% subplot(3,2,1); semilogy(freq,abs(frf(:,1))); xlabel('f [Hz]'); ylabel('|fft(A1)|');  title('Accelerometer 1 - DH1');
% subplot(3,2,3); semilogy(freq,abs(frf(:,2))); xlabel('f [Hz]'); ylabel('|fft(A2)|');  title('Accelerometer 2 - DH1');
% subplot(3,2,5); semilogy(freq,abs(frf(:,3))); xlabel('f [Hz]'); ylabel('|fft(A3)|');  title('Accelerometer 3 - DH1');
% 
...
load('DH2.mat')
% figure(2)
% subplot(3,2,1); semilogy(freq,abs(frf(:,1))); xlabel('f [Hz]'); ylabel('|fft(A1)|');  title('Accelerometer 1 - DH2'); 
% subplot(3,2,3); semilogy(freq,abs(frf(:,2))); xlabel('f [Hz]'); ylabel('|fft(A2)|');  title('Accelerometer 2 - DH2');
% subplot(3,2,5); semilogy(freq,abs(frf(:,3))); xlabel('f [Hz]'); ylabel('|fft(A3)|');  title('Accelerometer 3 - DH2');
% 
...
% load('RH1.mat')
% figure(3)
% subplot(3,2,1); semilogy(freq,abs(frf(:,1))); xlabel('f [Hz]'); ylabel('|fft(A1)|');  title('Accelerometer 1 - RDH1');
% subplot(3,2,3); semilogy(freq,abs(frf(:,2))); xlabel('f [Hz]'); ylabel('|fft(A1)|');  title('Accelerometer 2 - RDH1');
% subplot(3,2,5); semilogy(freq,abs(frf(:,3))); xlabel('f [Hz]'); ylabel('|fft(A1)|');  title('Accelerometer 3 - RDH1');
% 
% subplot(3,2,2); plot(freq,angle(frf(:,1))); xlabel('< [rad]'); ylabel('<fft(A1)');  title('Accelerometer 1 - RDH1');
% subplot(3,2,4); plot(freq,angle(frf(:,2))); xlabel('< [rad]'); ylabel('<fft(A2)');  title('Accelerometer 2 - RDH1');
% subplot(3,2,6); plot(freq,angle(frf(:,3))); xlabel('< [rad]'); ylabel('<fft(A3)');  title('Accelerometer 3 - RDH1');
...
[picchi,pos]=findpeaks(ch1+ch2+ch3,'minpeakheight',100,'minpeakdistance',100);
nat_freq=freq(pos)
...
% shape of the mode. (Slide 12/12)
% pti(:,1)=ch1(pos)./angle(frf(pos,1));
% pti(:,2)=ch2(pos)./angle(frf(pos,2));
% pti(:,3)=ch3(pos)./angle(frf(pos,3));
% pti1=pti(:,1)./(max(abs(pti(:,1))*max(modes.y)));
% pti2=pti(:,2)./(max(abs(pti(:,2))));
% pti3=pti(:,3)./(max(abs(pti(:,3))));
pti(:,1) = real(frf(pos,1)); 
pti(:,2) = real(frf(pos,2));
pti(:,3) = real(frf(pos,3));

% Normalization 
% for ii=1:length(nat_freq)
%     switch ii
%         case 1 % every mode of vibration is divided for the maximum
%         between
%                % all the modes 'max(abs(pti(ii,:))' and multiplied for
%                it's own maximum
...
%                % all the modes 'max(abs(pti(ii,:))' and multiplied for
%                it's own maximum
%             pti(ii,:)=-pti(ii,:)/max(abs(pti(ii,3)))*max(modes.y(10:end-10,ii));
%         case 2
%             pti(ii,:)=pti(ii,:)/max(abs(pti(ii,2)))*max(modes.y(10:end-10,ii));
%             % 10:end-10 is needed to correct the plot after normalization
%         case 3
%             pti(ii,:)=-pti(ii,:)/max(abs(pti(ii,3)))*max(modes.y(10:end-10,ii));
%         case 4
%             pti(ii,:)=-pti(ii,:)/max(abs(pti(ii,2)))*max(modes.y(10:end-10,ii));
...
for ii = 1:5
    % length(modes) = 121, L_beam = 1.2m => one point every 0.01m.
    % 42, 43 and 61 are the point in wich analytical displacement is
    % considered for normalization (position of A1, A2 and A3).
    lambda = (modes.y(43,ii)+modes.y(42,ii))/(2*pti(ii,2));
    if ii==3
        lambda=modes.y(61,ii)/pti(ii,3);
    end
    pti(ii,:)=lambda*pti(ii,:);
end
